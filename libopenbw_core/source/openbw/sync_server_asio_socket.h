...
struct sync_server_asio_socket {
	...
	void write_handler(client_t* c, const boost::system::error_code& ec, size_t bytes_transferred) {
		if (ec) {
	...
	void set_timeout(duration_T&& duration, callback_F&& callback) {
		...
		timer.async_wait([callback = std::forward<callback_F>(callback)](const boost::system::error_code& ec) {
			if (!ec) callback();
		});
	}
	...
	void read_handler(client_t* c, const boost::system::error_code& ec, size_t bytes_transferred) {
		if (ec) {
			if (c->on_kill) c->on_kill();
		} else {
