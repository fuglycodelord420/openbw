...
struct sync_server_asio_tcp: sync_server_asio_socket<asio::ip::tcp::socket> {
	...
	void accept_handler(const boost::system::error_code& ec, std::shared_ptr<acceptor_t> acceptor) {
	...
	void connect(const asio::ip::tcp::endpoint& ep) {
		...
		auto* s = &*socket;
		s->async_connect(ep, [this, socket = std::move(socket)](const boost::system::error_code& ec) {
			if (!ec) {
